...
<template>
  <div>
    <img :src="imageUrl" alt="" class="head" @error="handleImageError"/>
  </div>
</template>
...
export default {
  ...
  methods: {
    handleImageError() {
      this.imageUrl = require('@/assets/xxx.png')
    }
  }
}
</script>
...
## 说明
* 实现思路：使用img标签中的@error事件动态改变src路径，注意，若是本地图片注意使用require函数